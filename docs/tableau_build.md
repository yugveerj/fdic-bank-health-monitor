# Tableau Public satellite — exact build script

Tableau is the one GUI-locked tool in the stack, so this is the spec's
sanctioned delivery path: a click-by-click script executed mechanically in
Tableau Desktop (Public Edition). Every design decision is already made —
if a step is ambiguous, that's a bug in this document.
