...
## Path A (try this first): open the generated workbook

The workbook is authored directly as XML — `tableau/fdic_peer_satellite.twb`,
generated by `tableau/build_twb.py` against the structure of real Tableau
2026.1 workbooks using the Google Drive connector, and adversarially reviewed
against nine Desktop-authored ground-truth files. Your part:

1. Open `tableau/fdic_peer_satellite.twb` in Tableau Desktop Public Edition.
2. Desktop will prompt for Google sign-in (the connection embeds no
   credential by design) — sign in with the account that owns the Sheet.
3. Check the two dashboards render: **Peer explorer** (distribution +
   tails table + band/metric filters) and **Bank profile** (two trend
   panels + bank selector). Cosmetic nudges are fine to do by hand.
4. **File → Save to Tableau Public As…** → name it
   `FDIC Bank Health Monitor — Peer Satellite`.
5. On the published page: Edit Details → enable **Keep this data in sync**
   (the daily refresh), then send me the public URL.

If Desktop refuses the file or something renders too broken to nudge, fall
back to Path B below and tell me what Desktop complained about.
